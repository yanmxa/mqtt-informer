...
type Reflector struct {
	...
	// cluster identifier -> lastResourceVersion/isUnavailable/lock
	lastSyncResourceVersions map[string]lastResourceVersion
...
// 	indexer = NewIndexer(MetaNamespaceKeyFunc, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc})
...
func NewReflector(lw cache.ListerWatcher, expectedType interface{}, store Store, resyncPeriod time.Duration) *Reflector {
...
// information.
func NewNamedReflector(name string, lw cache.ListerWatcher, expectedType interface{}, store Store, resyncPeriod time.Duration) *Reflector {
...
// change.
func NewReflectorWithOptions(lw cache.ListerWatcher, expectedType interface{}, store Store, options ReflectorOptions) *Reflector {
...
func (r *Reflector) ListAndWatch(stopCh <-chan struct{}) error {
	...
	// watchlist the for every cluster's the lastresourceversion

	// every time receive a cluster joined event, then watchlist the cluster
...
func (r *Reflector) watch(w watch.Interface, stopCh <-chan struct{}) error {
	...
	for {
		...
		err = watchHandler(start, w, r.store, r.expectedType, r.expectedGVK, r.name, r.typeDescription, r.setLastSyncResourceVersion, nil, r.clock, stopCh)
...
func (r *Reflector) watchList(stopCh <-chan struct{}) (watch.Interface, error) {
	...
	for {
		...
		temporaryStore = NewStore(cache.DeletionHandlingMetaNamespaceKeyFunc)
		...
		case event, ok := <-w.ResultChan():
			...
			if expectedType != nil {
				if e, a := expectedType, reflect.TypeOf(event.Object); e != a {
					utilruntime.HandleError(fmt.Errorf("%s: expected11 type %v, but watch event object had type %v", name, e, a))
					continue
				}
			}
			if expectedGVK != nil {
