...
type DeltaFIFO struct {
	...
	// retryOnError is whether to retry processing an object if the
	RetryOnError bool
}
...
var _ = cache.Queue(&DeltaFIFO{}) // DeltaFIFO is a Queue
...
func (f *DeltaFIFO) Pop(process cache.PopProcessFunc) (interface{}, error) {
	f.lock.Lock()
	defer f.lock.Unlock()
	for {
		for len(f.queue) == 0 {
			...
			if f.closed {
				return nil, cache.ErrFIFOClosed
			}
